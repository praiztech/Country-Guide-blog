...
function createSuggestedOptions(suggestionsArray) {
  ...
  for (let i = 0; i < suggestionsArray.length; i++) {
    ...
    suggestedOption.addEventListener(
      "pointerenter",
      handlePointerDisplayVisualFocus
    );
    suggestedOption.addEventListener(
...
function openSuggestionsList(suggestionsList, suggestionsArray, comboboxInput) {
  ...
  setTimeout(
    () =>
      suggestionsWrapper.previousElementSibling.dispatchEvent(
        new CustomEvent("announce", {
          detail: `${suggestionsArray.length} suggestion${
            suggestionsArray.length > 1 ? "s" : ""
          } available`,
        })
      ),
    0
...
function modifySuggestionsList(
  suggestionsList,
  suggestionsArray,
  comboboxInput
) {
  ...
  while (suggestionsList.childElementCount > 0)
...
function handleSuggestionsListDisplay(comboboxInput) {
  const [suggestionsList, suggestionsListIsClosed] =
    getSuggestionsListRef(comboboxInput);
  const suggestionsArray =
    comboboxInput.value !== "" &&
    findCountrySuggestions(comboboxInput.value.normalize("NFC").toLowerCase());
  if (!suggestionsArray || suggestionsArray.length === 0) {
    // no suggested countries
...
function handleSuggestionsListClick(
  selectedOption,
  triggerMutationObserver = true
) {
  const selectedValue = selectedOption.textContent;
  const comboboxInput = selectedOption
    .getRootNode()
    ...
    case "Tab":
      {
        ...
        const selectedOptionId = comboboxInput.getAttribute(
        ...
        if (selectedOptionId !== null) {
          ...
          handleSuggestionsListClick(
            shadowNode.getElementById(selectedOptionId),
            false
          );
